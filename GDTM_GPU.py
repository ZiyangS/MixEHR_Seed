...
class GDTM(nn.Module):
    def __init__(self, K, corpus:Corpus, seeds_topic_matrix: torch.tensor, out='./store/'):
        """
        Arguments:
            K: Number of topics
        ...
        self.M = self.D # D in mini batch
        self.K = K
        self.T = corpus.T
        self.V = corpus.V  # vocabulary size of regular words # todo: check corpus.py's V, currently it looks like same with V in main.
        self.seeds_topic_matrix = seeds_topic_matrix.to(device) # V x K tensor, if exists value which indicates seed word w (row w) for topic k (column k)
    ...
    def inference_SCVB_SGD(self, args, max_epoch=10, save_every=100):
        ...
        for epoch in range(0, max_epoch):
            ...
            for i, d in enumerate(self.full_batch_generator): # For each epoach, we sample a series of mini_batch data once
                ...
                batch_times = batch_times.long()
                batch_C = torch.from_numpy(np.array(batch_C)).to(device)

                batch_size = len(batch_docs)
                BOW_M = torch.zeros(batch_size, self.V, dtype=torch.int, requires_grad=False, device=device) # x V
                # todo: batch_docs should be tensor BOW, M X V
                # todo: move to corpus
                for d_i, doc in zip(batch_indices, batch_docs):
                    for word_id, freq in doc.words_dict.items():
                        BOW_M[d_i, word_id] = freq # update BOW[d, w] = freq
                rnn_input = self.get_rnn_input(BOW_M, batch_times) # obtain T x V input for rnn
                start_time = time.time()
    ...
    def get_rnn_input(self, BOW_M, batch_times):
        '''
        get rnn input from documents
        :param BOW_M: BOW representation for a batch of documents
        ...
        for t in range(self.T):
            ...
            docs_t = BOW_M[tmp].squeeze().sum(dim=0)
            rnn_input[t] += docs_t
            cnt[t] += len(tmp)
