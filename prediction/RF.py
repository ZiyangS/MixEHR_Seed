...
def RF(X, label, diseases, f=None):
    ...
    for i, dis in enumerate(diseases):
        ...
        clf_cv = GridSearchCV(RandomForestClassifier(), parameters, scoring='average_precision', cv=spliter.split(X, y), n_jobs=-1)
        clf_cv.fit(X, y)
...
def test_RF(model_name, model, test_X, test_label, diseases, f=None):
    print('test of {}'.format(model_name))
    AUPRCs, AUROCs = [], []
    for i, dis in enumerate(diseases):
        prob = model[i].predict_proba(test_X)[:, 1]
