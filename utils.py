...
def tokenize_phecode_icd_corpus(corpus_data):
    ...
    '''
    ICD10_list = corpus_data.ICD10.unique()
    phecode_list = corpus_data.PheCode.unique()
    V, K = len(ICD10_list),  len(phecode_list) # V 12156, S 6954, K 1548
    # print(ICD10_list)
    # print(phecode_list)
    # print(V, K)
    phecode_icd_df = corpus_data.groupby(['ICD10', 'PheCode']).size().reset_index().rename(columns={0: 'count'})
    phecode_icd_dict = {}
    for index, row in phecode_icd_df.iterrows():
        if row['PheCode'] not in phecode_icd_dict.keys():
            phecode_icd_dict[row['PheCode']] = [row['ICD10']]
        else:
            phecode_icd_dict[row['PheCode']].append(row['ICD10'])
