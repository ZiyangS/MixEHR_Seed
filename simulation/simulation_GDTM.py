...
class GDTM(nn.Module):
    ...
    def initialize_tokens(self):
        for i, d in enumerate(self.full_batch_generator):
            batch_docs, batch_indices, batch_times = d # batch_Cj is total number of minibatch
    ...
    def get_kl_z(self, batch_times):
        ...
        kl_z -= np.sum(p_z)
        # E_q[log p(w | z, beta, mu, pi)], avoid overflow
    ...
    def CVB0(self, docs, indices, times):
    ...
    def infer_ind(self, docs, indices):
        infer_x = {doc.doc_id: np.zeros(len(doc.words_dict), self.K) for doc in docs}  # D x N_d
        infer_z = {doc.doc_id: np.zeros(len(doc.words_dict), self.K) for doc in docs}  # D x N_d
        for d_i, doc in zip(indices, docs):
            for w_i, (word_id, freq) in enumerate(doc.words_dict.items()): # w_i is the index of word in each document
                all_gamma = np.stack((self.gamma_ss[doc.doc_id][w_i],self.gamma_sr[doc.doc_id][w_i],self.gamma_rr[doc.doc_id][w_i]), axis=1)
    ...
    def CVB0_generative(self, docs, indices, times):
        # E step
        for d_i, doc in zip(indices, docs):
            # (the number of words in a doc) x K, not use V x K to save space
            temp_gamma_rr = np.zeros((len(doc.words_dict), self.K))
    ...
    def update_hyperparams(self, docs):
        '''
        update hyperparameters \pi, this is not useful as gamma_r_sum is extremely large, use indicator x
        '''
        eps = np.finfo(float).eps
    ...
    def inference_SCVB_AEVI(self, args, max_epoch=100, save_every=100):
        '''
        ...
        for epoch in range(max_epoch):
            ...
            for i, d in enumerate(self.full_batch_generator): # For each epoach, we sample a series of mini_batch data once
                ...
                batch_docs, batch_indices, batch_times = d  # batch_C is total number of words within a minibatch, used for estimation of SCVB0
                ...
                SGD_loss = kl_eta + kl_z
                SGD_loss.backward()
                self.optimizer.step()
                print(SGD_loss)
                print("\t took %s seconds" % (time.time() - start_time))
                elbo.append(SGD_loss.detach().cpu().numpy().item())
                print(elbo)
                print("epoch %s: elbo %s, diff %s "% (epoch, elbo[-1], np.abs(elbo[-1] - elbo[-2])))
                # theta_dk = np.zeros([self.D, self.K])
                # for d in range(self.D):
                #     t_d = batch_docs[d].t_d
                #     for k in range(self.K):
                #         theta_dk[d][k] = (self.exp_m[d][k] + self.alpha[t_d][k]) / (
                #                     self.exp_m_sum[d] + np.sum(self.alpha))
                # print(theta_dk)
                # pd.DataFrame(theta_dk).to_csv("simulation/inferred_theta/theta_infer%d.csv" % epoch, index=False)
